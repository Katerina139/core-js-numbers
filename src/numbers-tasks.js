...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorModules1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vectorModules2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(dotProductOfVectors / (vectorModules1 * vectorModules2));
}
